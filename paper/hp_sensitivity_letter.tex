...
\item It reproduces the aggregation effect on a second, previously unseen dataset drawn from a different climate and building stock, using a nested sequence of aggregates and without recalibrating the fitting procedure.

\end{enumerate}

\item \textbf{Roadmap.}

The rest of the paper is structured as follows: Section~\ref{section:identify} formalises the thermal response of aggregated net load and the two opposing effects of consumer aggregation; Section~\ref{section:usecases} describes the data and reports the two use cases against aggregation level; Section~\ref{section:validation} reproduces the aggregation effect on an unseen dataset; in Section~\ref{section:discussion}, accuracy, generalisability, data requirements and interpretability are discussed. Lastly, Section~\ref{section:conclusion} concludes the paper.
...
In this section, the thermal response of aggregated Low Voltage (LV) net load is formalised. Consumer aggregation is then shown to act on this response in two opposing directions, and the two use cases implemented in the remainder of this paper are derived from the tension between them.


\subsection{Thermal Response of Aggregated Net Load}

Electric Thermostatic Loads (ETLs), a class comprising Heat Pumps (HPs), direct resistive heating, and electric cooling, modulate their electrical consumption in response to ambient temperature. At the individual consumer level this behaviour is thermostatically controlled and strongly intermittent: a single ETL is either running or idle, and its instantaneous demand is a poor indicator of the temperature that caused it. Aggregation over the consumers downstream of an LV substation removes that intermittency. Thermostatic cycling decorrelates between dwellings and idiosyncratic occupancy averages out, so the aggregate response becomes smooth and approximately piecewise linear in temperature.
...
The first effect of increasing the consumer count $N$ is to improve how well Equation~\eqref{eq:thermal-response} describes the data. Averaging over more consumers suppresses the idiosyncratic component of demand while leaving the temperature-driven component intact, since only the latter is common to every consumer downstream of the substation. The fitted parameters are correspondingly better determined. Section~\ref{section:usecases} quantifies the improvement in the fit and in the spread of the parameters alike, and in each the gain is largest where ETL penetration is lowest.
...
These two effects do not carry equal weight. What Equation~\eqref{eq:sensitivity-decomposition} obstructs is not identification at a large $N$, but identification at an unknown $N$: were the consumer count published alongside the aggregated consumption, the reference contribution $N \sigma_{\mathrm{base}}$ could be subtracted and the ambiguity would not arise. Section~\ref{section:usecases} separates the two, by comparing substations of equal and known size against the same substations pooled over an unknown one.
...
Aggregation over time acts on the same trade-off, and for the same reason. Fitting is performed on daily aggregates rather than at the native measurement resolution, since averaging over a full diurnal cycle removes occupancy-driven variability that is not temperature-dependent. It also removes the effect of buildings' thermal inertia, which causes a mismatch between consumption and outdoor temperature at high resolutions. This averaging comes at the cost of intra-day detail. Consumer count and averaging window are two axes of one operation, and Section~\ref{section:usecases} reports the fit against both.
...
\textbf{Energy Estimation.} Integrating Equation~\eqref{eq:thermal-response} over the temperature profile of a given day yields the temperature-driven energy consumed on that day. For the heating arm, this reduces to the product of $s_h$, the HD value of that day, and the number of hours over which it applies. The cooling arm follows from the equivalent product using CD. Both terms of Equation~\eqref{eq:sensitivity-decomposition} contribute to the result, so the estimate refers to the total temperature-driven energy. Isolating the ETL share requires subtracting the reference contribution $N \sigma_{\mathrm{base}}$. The dead band also bounds where the estimate can be accurate at all: above $T_h$ the thermal term is identically zero, so any ETL energy that persists into mild weather is unexplained by construction, not by a fitting failure. Section~\ref{section:usecases} checks accuracy against both ETL penetration and the observed temperature range accordingly.

Both use cases are consequently reported against the aggregation level in Sections~\ref{section:usecases} and~\ref{section:validation}, rather than at a single one.
...
\section{Use Case Implementation and Results}
\label{section:usecases}
In this section, the data used to implement the two use cases introduced in Section~\ref{section:identify} are described, and the results obtained are presented.


\subsection{Data and Substation Construction}
\label{subsection:data}

The use cases are implemented on the HEAPO dataset, complemented with additional smart-meter data providing submetered thermal load. Both are Swiss, and together give net load at 15-minute resolution for 1702 LV consumers over calendar year 2023. Of these, 395 have an ETL, of which 81 carry a submetered heat pump. Heat pumps are the only ETL used in this paper, since they are the only ETL technology with a submetered thermal series in either dataset: the heat pump is the only device paired to a household meter in this data. The remaining 314 ETL-equipped consumers are identified from survey flags alone and are not used in the substation design below. Ambient temperature is available hourly, which sets 1~h as the finest resolution at which the fit is evaluated. No consumer has an electric cooling load, so $s_c$ is zero throughout and only the heating arm of Equation~\eqref{eq:thermal-response} is fitted.
...
\caption{Left: ROC curves for the raw thermal sensitivity and for the two normalised variants, training dataset. Right: detection rate against ETL penetration at 95\% specificity.}
\label{fig:detection}
\end{figure*}
...
Pooled across 438\,000 substation-days, the thermal term of Equation~\eqref{eq:thermal-response} reaches $R^2 = 0.953$ against submetered thermal energy, at a total-energy bias of $-11.5\%$ and an RMSE of 91~kWh/day. The estimate is built from the day's mean temperature alone, and is compared against a quantity the fit never saw, since the fit target was net load rather than thermal energy.
...
Accuracy is also not uniform across the temperature range. Splitting substation-days at the median daily mean temperature, the estimator reaches $R^2 = 0.971$ and a bias of $-2.4\%$ on cold days, against $R^2 = 0.453$ and a bias of $-54.7\%$ on warm days; RMSE is comparatively stable, at 82 and 99~kWh/day respectively. This is possibly a consequence of the dead band in Equation~\eqref{eq:thermal-response}: above $T_h$ the thermal term is exactly zero by construction, so any ETL energy that persists into mild weather -- domestic hot water, most plausibly -- is entirely unexplained by the estimator, and a large relative bias results even where the absolute error stays modest.


\subsection{Effect of Temporal Resolution}
\label{subsection:resolution}

Fit quality and detectability do not favour the same resolution. Median net-load $R^2$ improves monotonically as the averaging window widens, from 0.726 at 1~h to 0.948 at 24~h for a substation of 48 consumers at 50\% penetration, the uppermost line of Fig.~\ref{fig:aggregation-quality}. Detection AUC does not follow: it peaks at 4--8~h, at 0.994, and falls to 0.990 at 24~h (Fig.~\ref{fig:auc-resolution}). Resampling the same substations at every resolution, so that the comparison stays paired, confirms the difference is systematic: AUC at 4~h, 8~h and 16~h is significantly above the 24~h value, while no resolution differs significantly from 24~h in sensitivity.
...
\section{Reproduction on an Independent Dataset}
\label{section:validation}
In this section, the central claim of Section~\ref{subsection:fitquality} is tested on a second dataset, previously unseen and drawn from a different climate and building stock: that aggregating consumers improves the fit of Equation~\eqref{eq:thermal-response}.


\subsection{Dataset and Design}
\label{subsection:validation-data}

The dataset is WPUQ, 37 German single-family houses on one low-voltage feeder, each with separately metered heat pump and household circuits, over calendar year 2019. Ambient temperature is recorded at five-minute resolution against the training dataset's hourly series, though every fit in this paper rests on hourly temperature at the coarsest, so the difference does not propagate into the results. The data predates the German regulatory provisions for controllable heat pump loads that came into force in 2024.

No consumer in WPUQ has a labelled cooling load, and the summer behaviour of its heat pumps is consistent with this. Pooling the 37 units over June-August, the correlation between load and temperature is negative in 18 of 37 cases at $p<0.05$ against positive in 2, and the median correlation across all units is $-0.18$. Only the heating arm of Equation~\eqref{eq:thermal-response} is fitted here as well.

The scope of what WPUQ can test is set by what it contains. Every household carries a heat pump, so a reference group of genuinely ETL-free consumers cannot be assembled: withholding a household's own heat pump circuit would yield a negative class that differs from the positive one by that circuit alone, with the entire non-thermal load held identical. Detection scored against such a class would be an easier problem than the one Section~\ref{subsection:detection} poses, and would report an optimistic figure for that reason rather than because the threshold transfers. Detection is consequently not evaluated here, and the figures of Section~\ref{subsection:detection} stand on the training dataset alone.

The aggregation claim carries no such difficulty, since it concerns how the fit responds to consumer count rather than how two classes separate. It is tested with one substation per aggregation level, in a nested sequence: the aggregate at $N$ contains the aggregate at $N-4$, so each step is the addition of four consumers to a fixed set rather than a comparison between independent draws. Roles are fixed before any aggregate is built, half the pool contributing its heat pump circuit and half having it withheld, which holds penetration at 50\% throughout and matches the condition shown for the training dataset in Fig.~\ref{fig:aggregation-quality}.

Nine substations result, from $N = 4$ to $N = 36$. No replicates are drawn and none are implied. A factorial design over the same 37 houses would yield a larger substation count without a corresponding gain in evidence, for the reason set out in Section~\ref{subsection:data}. The single nested sequence instead makes the dependence on $N$ visible without inviting a precision claim the pool cannot support.

The feeder also carries a measured transformer, against which the synthetic aggregation can be checked. The 37 monitored households account for 49.5\% of the transformer's mean load and the two series correlate at 0.95. The monitored set consequently behaves as a substantial and representative fraction of a real feeder, rather than as an arbitrary collection of houses.


\subsection{Fit Quality Against Aggregation Level}
\label{subsection:validation-results}

Fit quality rises with the consumer count at every averaging window (Fig.~\ref{fig:nested-aggregation}). At 24~h, net-load $R^2$ goes from 0.667 at $N = 4$ to 0.845 at $N = 36$, and at 1~h from 0.499 to 0.605. The direction reported in Section~\ref{subsection:fitquality} is therefore reproduced on a dataset the fitting procedure has never seen, with no recalibration of bounds and no adjustment to the fit.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_nested_aggregation.pdf}
\caption{Net-load $R^2$ against consumer count for the nested WPUQ sequence, at 50\% ETL penetration. Each point is a single substation, and the substation at $N$ contains the one at $N-4$, so the horizontal axis is the addition of consumers to a fixed aggregate. No band is shown because there are no replicates.}
\label{fig:nested-aggregation}
\end{figure}

The rise is not strictly monotone. $R^2$ falls slightly between $N = 12$ and $N = 16$ at the finer windows, and between $N = 32$ and $N = 36$ at every window, which is expected of single realisations: an individual consumer added at one step may be less regular than the aggregate it joins. The training dataset shows the same departures where its own cells are read individually, and they are smoothed there only by taking a median over replicates.

The two aggregation axes interact differently here, and in the opposite sense. In Switzerland the consumer-count effect shrinks as the window widens, from 0.410 at 1~h to 0.092 at 24~h, which is what makes the two axes substitutes there. In WPUQ it grows instead, from 0.106 at 1~h to 0.178 at 24~h. The substitution reported in Section~\ref{subsection:fitquality} therefore does not reproduce. This might indicate that the WPUQ fits remain limited at fine resolution by something other than consumer count, its houses being individually metered single-family dwellings with sharper sub-daily swings than a mixed consumer pool. The levels are also uniformly lower, at 0.845 against 0.948 for the largest aggregate at 24~h, yet the direction of the dependence on $N$ holds at every window.

What this establishes is narrower than a validation of the method. The aggregation behaviour on which Sections~\ref{section:identify} and~\ref{section:usecases} rest is not an artefact of one dataset, one climate or one building stock. Whether the Detection and Energy Estimation results transfer is a separate question, which WPUQ is not constituted to answer.
...
Detection needs, in addition, a comparison point: either a local reference distribution built from substations known to carry no ETL, as in Section~\ref{subsection:data}, or an externally calibrated threshold. Whether a threshold calibrated elsewhere can stand in for a local reference group is not settled here, since Section~\ref{section:validation} cannot supply an ETL-free population to test it against. What Detection does not need is the consumer count $N$ itself, since the statistic it operates on is already normalised by peak or base load.

Energy Estimation needs neither a reference group nor $N$ to report the total temperature-driven energy of a substation, the quantity validated against submetered ground truth in Section~\ref{section:usecases}. $N$ becomes necessary only if the ETL-specific share is required rather than the total, since isolating it means subtracting the reference-group contribution $N \sigma_{\mathrm{base}}$ from Equation~\eqref{eq:sensitivity-decomposition}, as noted in Section~\ref{section:identify}.
...
The same caution applies on the warm side, where Section~\ref{subsection:accuracy} attributes part of the Energy Estimation bias to domestic hot water demand that persists above $T_h$. The fitted zero above $T_h$ should accordingly be read as \emph{not captured by this term}, rather than as evidence that no ETL load remains, since the model has no mechanism to distinguish the two.

Some physically meaningful quantities are not recoverable from Equation~\eqref{eq:thermal-response} at any temperature, extrapolated or not. Section~\ref{section:identify} notes that installed ETL capacity requires the ratio between aggregated demand and the sum of individual rated powers, which net load alone does not identify. Interpretability, in this sense, is bounded twice over: once by the observed temperature range, and once by what the functional form of Equation~\eqref{eq:thermal-response} was ever capable of representing.
...
What aggregation buys is precision. Fit quality and Energy Estimation both improve with the consumer count, and most where ETL penetration is lowest, which is the regime a network operator meets first as uptake begins. What aggregation does not buy is correctness. The total-energy bias is invariant to $N$ across the whole range tested, since it follows from the functional form of Equation~\eqref{eq:thermal-response} rather than from the size of the sample.

What aggregation costs is attributability, and that cost is both measurable and largely recoverable. The same summation that smooths the response accumulates the non-ETL term of Equation~\eqref{eq:sensitivity-decomposition}, so a well-fitted curve is ambiguous about what produced it. At a known consumer count, the raw sensitivity separates ETL-bearing substations from ETL-free ones essentially perfectly, at every size tested. Pooled over an unknown count it does not, and normalising by peak load restores roughly three quarters of the difference.

Two limits remain that further aggregation does not address. The dead band bounds Energy Estimation on the warm side, where hot water demand persists that the thermal term cannot represent by construction. Installed capacity is not recoverable at all, under either reading and at any aggregation level, since it requires a ratio between aggregated demand and rated power that net load never exposes.

No single aggregation level is therefore preferable across both use cases. Larger substations yield better-determined fits and more accurate energy estimates, whereas Detection is already saturated at the smallest size tested and gains nothing further from size. Where a consumer count is available it is worth more than additional aggregation, and where it is not, the normalised statistic recovers most of what its absence costs. The latter is the condition under which a DSO actually operates.
